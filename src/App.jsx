    ...
    <>
      <div>
        ...
        <form onSubmit={handleClick} className="flex pl-23">
          <input onChange={handleChange} type="text" value={text} className="h-10 bg-white rounded-l-2xl" />
          <button onClick={handleClick} className="h-10 w-30 bg-blue-600 rounded-r-2xl font-bold" >Add-task</button>
        </form>
      </div>
      <div>
        <ul className="flex flex-col justify-center items-center list-none">
          <AddTask tasks={tasks} handleEdit = {handleEdit} handleDelete = {handleDelete} />
        </ul>
      </div>
    </>
  );
